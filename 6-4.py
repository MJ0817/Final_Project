...
df['UTC'] = pd.to_datetime(df['UTC'], unit='s')  # 유닉스 타임스탬프를 사람이 이해할 수 있는 형태로 변환
...
plt.plot(df['UTC'], df['Temperature[C]'], label='Temperature (C)')
plt.plot(df['UTC'], df['Humidity[%]'], label='Humidity (%)')
plt.xlabel('Time')
plt.ylabel('Values')
