...
# 8. 데이터 시각화
# 예시로 Temperature와 Fire Alarm의 관계를 시각화
plt.figure(figsize=(12, 6))
plt.plot(df['Temperature[C]'], label='Temperature[C]', color='orange')
plt.plot(df['Fire Alarm'] * 50, label='Fire Alarm', color='red', alpha=0.6)  # 화재 발생 여부를 시각적으로 강조
plt.title('Temperature and Fire Alarm over Time')
plt.xlabel('Time')
plt.ylabel('Temperature (C)')
plt.legend()
plt.show()

# 예시로 Humidity와 Fire Alarm의 관계를 시각화
plt.figure(figsize=(12, 6))
plt.plot(df['Humidity[%]'], label='Humidity[%]', color='blue')
plt.plot(df['Fire Alarm'] * 100, label='Fire Alarm', color='red', alpha=0.6)
plt.title('Humidity and Fire Alarm over Time')
plt.xlabel('Time')
plt.ylabel('Humidity (%)')
plt.legend()
plt.show()
